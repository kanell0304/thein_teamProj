...
public class MerchantClassificationProcessor {
    ...
    private Optional<MerchantUsageData> classifyMerchant(
        ...
        String placeName = normalizeText(selectedCandidate.placeName());
        String categoryName = normalizeText(selectedCandidate.categoryName());
     ...
     * @param bestCandidates 동일한 이름 일치 우선순위를 가진 장소 후보 목록
    ...
    private Optional<SearchCandidate> resolveCandidate(
        ...
        for (SearchCandidate candidate : bestCandidates) {
            if (candidate == null
                    ...
                    || normalizeText(candidate.placeName()) == null) {
                return Optional.empty();
            }

            String candidateCategoryName = normalizeText(candidate.categoryName());
        ...
        return merchantPlaceMatcher.selectUniqueMostSimilarCandidate(
                merchantName,
                bestCandidates
        );
    }
