...
/**
 * 옵션 계층에서 선택한 시각에 따라 필터링된 동일 가맹점의 결제와 카카오 장소 매칭 결과를 묶은 중간 데이터다.
 *
 ...
 *
 * <p>가맹점 집계 직후 생성하는 데이터는 {@link KakaoPlaceMatchData#empty()}를 사용한다.
 * 이후 장소 분류 단계는
...
public record MerchantUsageData(
     ...
     * @param merchantRegistrationNumber 사업자등록번호 기반 가맹점 코드
     * @param payments 승인시각·승인금액 결제 로그 목록
     * @return 분류 전 카카오 결과를 포함한 가맹점 사용 이력
     */
    public static MerchantUsageData withoutPlaceMatch(
            String merchantName,
            String merchantRegistrationNumber,
            List<PaymentLog> payments
    ) {
