 ...
 * 검증을 통과한 응답은 {@link ConsentedCardIdSelector}가 사용할 수 있다.
 */
@Component
public class CardListValidator {

    private static final String SUCCESS_RESPONSE_CODE = "00000";
    private static final Set<String> CARD_MEMBER_CODES = Set.of("1", "2");
    ...
    public void validate(CardListResponse response) {
        ...
        for (int index = 0; index < cards.size(); index++) {
            validateCard(cards.get(index), index, cardIds);
        }
    }
    ...
    private void validateCard(
        ...
        validateRequiredText(card.cardId(), "card_list[" + index + "].card_id");

        // 마스킹된 카드번호 card_num이 비어 있지 않은지 검사한다.
        validateRequiredText(card.maskedCardNumber(), "card_list[" + index + "].card_num");

        // 카드상품명 card_name이 비어 있지 않은지 검사한다.
        validateRequiredText(card.cardName(), "card_list[" + index + "].card_name");

        // 본인·가족 구분 코드 card_member가 비어 있지 않은지 검사한다.
        validateRequiredText(card.cardMemberCode(), "card_list[" + index + "].card_member");

        // 카드 구분 코드 card_type이 비어 있지 않은지 검사한다.
        validateRequiredText(card.cardTypeCode(), "card_list[" + index + "].card_type");

        // 전송요구 여부 is_consent가 true 또는 false로 명시됐는지 검사한다.
        if (card.transferConsented() == null) {
            throw new IllegalArgumentException(
        ...
        if (!CARD_MEMBER_CODES.contains(card.cardMemberCode())) {
            throw new IllegalArgumentException(
                    "card_member는 1 또는 2여야 합니다: " + card.cardMemberCode()
            );
        }

        // card_type이 신용 01, 체크 02, 소액신용체크 03 코드인지 검사한다.
        if (!CARD_TYPE_CODES.contains(card.cardTypeCode())) {
            throw new IllegalArgumentException(
                    "card_type은 01, 02, 03 중 하나여야 합니다: " + card.cardTypeCode()
            );
        }
    }
