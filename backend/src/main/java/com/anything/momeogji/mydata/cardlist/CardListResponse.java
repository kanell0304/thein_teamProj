...
public record CardListResponse(
     ...
     * @param consented 해당 카드의 개인신용정보 전송요구 여부
     * @param cardName 카드상품명
     * @param memberCode 본인·가족 구분 코드
     * @param typeCode 카드 구분 코드
     */
    @JsonIgnoreProperties(ignoreUnknown = true)
    public record CardItem(
            @JsonProperty("card_id") String cardId,
            @JsonProperty("card_num") String maskedCardNumber,
            @JsonProperty("is_consent") Boolean consented,
            @JsonProperty("card_name") String cardName,
            @JsonProperty("card_member") String memberCode,
            @JsonProperty("card_type") String typeCode
    ) {
    }
}
