...
import com.anything.momeogji.mydata.processing.MerchantNameNormalizer;
...
public class MerchantPlaceMatcher {
    ...
    private static final int MIN_PARTIAL_NAME_LENGTH_PERCENT = 60;
     ...
     * @param merchantNameNormalizer 원본과 외부 장소명에서 비교용 문자열을 생성하는 컴포넌트
     */
    ...
    public MatchResult match(
        ...
        List<SearchCandidate> firstExactMatches = findExactMatches(
                merchantName,
                merchantNameKey,
                firstSearchCandidates
        );
        if (!firstExactMatches.isEmpty()) {
            return MatchResult.ofMatches(firstExactMatches);
        }
        ...
        if (merchantNameKey.equals(merchantName)) {
        ...
        List<SearchCandidate> secondExactMatches = findExactMatches(
                merchantName,
                merchantNameKey,
                secondSearchCandidates
        );
        if (!secondExactMatches.isEmpty()) {
            return MatchResult.ofMatches(secondExactMatches);
        }

        // 두 검색 모두 부분일치만 있다면 원본 검색 결과를 우선한다.
        if (!firstPartialMatches.isEmpty()) {
            return MatchResult.ofMatches(firstPartialMatches);
        }

        // 1차 검색에서도 후보가 없었던 경우 2차 검색의 부분일치 여부로 최종 결과를 결정한다.
        return MatchResult.fromCandidates(
                findPartialMatches(merchantNameKey, secondSearchCandidates)
        );
    }
     ...
     */
    Optional<SearchCandidate> selectUniqueBestLengthMatch(
            String merchantName,
            List<SearchCandidate> candidates
    ) {
        String merchantNameKey = merchantNameNormalizer.toComparisonKey(merchantName);
        SearchCandidate mostSimilarCandidate = null;
        NameLengthRatio highestLengthRatio = null;
        boolean highestLengthRatioTied = false;

        // 모든 후보의 정규화된 장소명 길이 비율을 현재 최고 비율과 비교한다.
        for (SearchCandidate candidate : candidates) {
            String placeNameKey = toCandidateNameKey(candidate);
            if (placeNameKey == null) {
                continue;
            }

            NameLengthRatio candidateLengthRatio = NameLengthRatio.of(
                    merchantNameKey,
                    placeNameKey
            );

            // 첫 유효 후보 또는 기존 최고 비율보다 높은 후보를 새 최종 후보로 보존한다.
            if (highestLengthRatio == null
                    || candidateLengthRatio.compareTo(highestLengthRatio) > 0) {
                mostSimilarCandidate = candidate;
                highestLengthRatio = candidateLengthRatio;
                highestLengthRatioTied = false;
                continue;
            }

            // 교차 곱 결과가 정확히 같을 때만 최고 유사도 동률로 표시한다.
            if (candidateLengthRatio.compareTo(highestLengthRatio) == 0) {
                highestLengthRatioTied = true;
            }
        }

        // 유일한 최고 후보가 없으면 카테고리 충돌을 해소하지 못한 것으로 처리한다.
        if (mostSimilarCandidate == null || highestLengthRatioTied) {
     ...
     *
     * @param merchantName 마이데이터의 원본 가맹점명
     * @param merchantNameKey 원본 가맹점명의 비교용 문자열
     * @param candidates 현재 검색 회차에서 받은 장소 후보
     * @return 가장 우선하는 완전일치 후보 목록
     */
    private List<SearchCandidate> findExactMatches(
            String merchantName,
            String merchantNameKey,
            List<SearchCandidate> candidates
    ) {
        ...
        return candidates.stream()
                .filter(candidate -> merchantNameKey.equals(
                        toCandidateNameKey(candidate)
                ))
                .toList();
    }
     ...
     */
    private String toCandidateNameKey(SearchCandidate candidate) {
        try {
            return merchantNameNormalizer.toComparisonKey(candidate.placeName());
    ...
    private boolean isPartialMatch(String left, String right) {
        ...
        NameLengthRatio nameLengthRatio = NameLengthRatio.of(left, right);

        // 부동소수점 없이 짧은 이름이 긴 이름의 60% 이상인지 교차 곱으로 확인한다.
        return nameLengthRatio.meetsMinimumPercent(MIN_PARTIAL_NAME_LENGTH_PERCENT);
    }
     ...
     */
    private record NameLengthRatio(
            int shorterLength,
            int longerLength
    ) implements Comparable<NameLengthRatio> {
         ...
         */
        @Override
        public int compareTo(NameLengthRatio other) {
     ...
     *
     * @param priorityCandidates 외부 제공자 응답 순서를 유지한 최종 장소 후보 목록
     */
    public record MatchResult(List<SearchCandidate> priorityCandidates) {
        ...
        public MatchResult {
            ...
            if (priorityCandidates == null) {
                throw new IllegalArgumentException("priorityCandidates는 null일 수 없습니다.");
            }
            priorityCandidates = List.copyOf(priorityCandidates);
        }
         ...
         */
        private static MatchResult ofMatches(List<SearchCandidate> candidates) {
        ...
        private static MatchResult fromCandidates(List<SearchCandidate> candidates) {
            return candidates.isEmpty() ? unmatched() : ofMatches(candidates);
        }
        ...
        public boolean hasMatches() {
            return !priorityCandidates.isEmpty();
        }
    }
}
