package com.anything.momeogji.mydata.cardapproval;
...
public class CardApprovalValidator {
    ...
    private static final Set<String> TRANSACTION_DATE_TIME_REQUIRED_STATUS_CODES = Set.of("02", "03");
    private static final String MODIFIED_STATUS_CODE = "03";
    ...
    public void validate(CardApprovalResponse response) {
        ...
        validRequiredText(response.responseMessage(), "rsp_msg");
        ...
        for (int index = 0; index < approvals.size(); index++) {
            validApproval(approvals.get(index), index);
        }
    }
     ...
     */
    private void validApproval(CardApprovalResponse.ApprovalItem approval, int index) {
        ...
        validRequiredText(approval.approvalNumber(), fieldPrefix + ".approved_num");

        // 승인일시 approved_dtime이 필수이며 실제 존재하는 14자리 시각인지 검사한다.
        validRequiredDateTime(approval.approvedDateTime(), fieldPrefix + ".approved_dtime");

        // 결제상태 status가 필수이며 정의된 01·02·03·04 코드인지 검사한다.
        validRequiredText(approval.statusCode(), fieldPrefix + ".status");
        ...
        validRequiredText(approval.payTypeCode(), fieldPrefix + ".pay_type");
        ...
        validRequiredNonNegativeAmount(
                approval.approvedAmount(),
                fieldPrefix + ".approved_amt"
        );

        // 승인취소 02 또는 정정 03이면 trans_dtime이 필수이고 유효한 시각인지 검사한다.
        if (TRANSACTION_DATE_TIME_REQUIRED_STATUS_CODES.contains(approval.statusCode())) {
        ...
        if (MODIFIED_STATUS_CODE.equals(approval.statusCode())) {
            validRequiredNonNegativeAmount(
            ...
            validOptionalNonNegativeAmount(
                    approval.modifiedAmount(),
                    fieldPrefix + ".modified_amt"
            );
        }
        ...
        validOptionalText(approval.merchantName(), fieldPrefix + ".merchant_name");
     ...
     */
    private void validRequiredText(String value, String fieldName) {
     ...
     */
    private void validOptionalText(String value, String fieldName) {
     ...
     */
    private void validRequiredDateTime(String value, String fieldName) {
        // 필수 일시 문자열이 누락됐거나 공백인지 먼저 검사한다.
        validRequiredText(value, fieldName);

        // 14자리 일시 문자열이 달력상 실제 존재하는 시각인지 엄격하게 검사한다.
        validDateTimeFormat(value, fieldName);
    }
     ...
     */
    private void validOptionalDateTime(String value, String fieldName) {
     ...
     */
    private void validDateTimeFormat(String value, String fieldName) {
        try {
     ...
     */
    private void validRequiredNonNegativeAmount(BigDecimal amount, String fieldName) {
        ...
        validOptionalNonNegativeAmount(amount, fieldName);
    }
     ...
     */
    private void validOptionalNonNegativeAmount(BigDecimal amount, String fieldName) {
