...
class MerchantUsageProcessorTest {
    ...
    private final MerchantUsageProcessor merchantUsageProcessor =
            new MerchantUsageProcessor(new MerchantNameNormalizer());
    ...
    void 선택_시각과_승인_시각을_내부_시간대_경계에_맞춰_필터링한다() {
        List<CleanedApprovalData> approvals = List.of(
                approval("01:59", 1, 59),
                approval("02:00", 2, 0),
                approval("09:59", 9, 59),
                approval("10:00", 10, 0),
     ...
     */
    private CleanedApprovalData approval(String label, int hour, int minute) {
